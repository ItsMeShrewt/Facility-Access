<?php
    ...
    if ($instructor == "" || $course == "" || $section == "" || $subject == "" || $date == "" || $start == "" || $end == "" || $room == "") {
        ...
        $checkQuery = "SELECT * FROM insroom WHERE room = '$room' AND date = '$date' AND (
        ...
        if (mysqli_num_rows($result) > 0) {
           ...
           $insertQuery = "INSERT INTO insroom(instructor, course, section, subject, date, start, end, reason, room) 
                            VALUES('$instructor', '$course', '$section', '$subject', '$date', '$start', '$end', '$reason', '$room')";
